...
form.addEventListener('submit', (e) => {
  // Accessibilité : empêche le rechargement et garde l’utilisateur dans le contexte de la modale.
  e.preventDefault();
  validate(e);
});
...
function validateName(input) {
  ...
  if (!regex.test(input.value)) {
    ...
    // Accessibilité : indique le champ invalide aux lecteurs d’écran.
    input.setAttribute('aria-invalid', 'true');
    return false;
  } else {
    input.classList.remove("field-error");
    // Accessibilité : remet l’état ARIA à valide après correction.
